...
public class ShopsBridge_ExcellentShop4_20 implements IShopsBridge {

    private final CompletableFuture<Void> readyFuture = new CompletableFuture<>();

    public ShopsBridge_ExcellentShop4_20(Plugin plugin) {
                ...
                if (virtualShop.isTradeAllowed(tradeType)) {
                    for (VirtualProduct virtualProduct : virtualShop.getProducts()) {
                        ...
                        if (content instanceof ItemContent) {
                            if (!((ItemContent) content).isItemMatches(itemStack))
                                continue;
                        }
