...
public class ShopsBridge_ExcellentShop5_0 implements IShopsBridge {

    private final CompletableFuture<Void> readyFuture = new CompletableFuture<>();

    private final VirtualShopModule virtualShopModule;

    public ShopsBridge_ExcellentShop5_0(Plugin plugin) {
        Scheduler.runTaskLater(plugin, () -> this.readyFuture.complete(null), 1L);
        ShopPlugin shopPlugin = JavaPlugin.getPlugin(ShopPlugin.class);
        this.virtualShopModule = (VirtualShopModule) shopPlugin.getModuleRegistry().getById("virtual_shop");
    }
    ...
    private Optional<VirtualProduct> getVirtualProductFor(ItemStack itemStack, TradeType tradeType, @Nullable Player player) {
        return player == null ? getShopProduct(itemStack, tradeType) :
                Optional.ofNullable(this.virtualShopModule)
    ...
    private Optional<VirtualProduct> getShopProduct(ItemStack itemStack, TradeType tradeType) {
        return Optional.ofNullable(this.virtualShopModule).map(virtualShopModule -> {
            for (VirtualShop virtualShop : virtualShopModule.getShops()) {
                if (virtualShop.isTradeAllowed(tradeType)) {
                    for (VirtualProduct virtualProduct : virtualShop.getProducts()) {
                        ProductContent content = virtualProduct.getContent();
                        if (content instanceof ItemContent) {
                            if (!content.isItemMatches(itemStack))
                                continue;
                        }
    ...
    private static TransactionImpl getSellTransactionFor(VirtualProduct product, @Nullable Player player, ItemStack itemStack) {
        double sellPrice = product.getFinalSellPrice(player, itemStack.getAmount());
        return new TransactionImpl(player, Transaction.Type.SELL, itemStack, BigDecimal.valueOf(sellPrice), product);
    }

    private static TransactionImpl getBuyTransactionFor(VirtualProduct product, @Nullable Player player, ItemStack itemStack) {
        double buyPrice = product.getFinalBuyPrice(player, itemStack.getAmount());
        return new TransactionImpl(player, Transaction.Type.BUY, itemStack, BigDecimal.valueOf(buyPrice), product);
    }
    ...
    private static class TransactionImpl extends AbstractTransaction {
        ...
        public void onTransact() {
            if (this.product == null || product.getPricingType() != PriceType.DYNAMIC)
                return;
            ...
            ERawTransaction transaction = ERawTransaction.builder(player, tradeType)
                    .setPreview(false)
                    .setStrict(false)
                    .addItem(getItem())
                    .build();
            virtualShop.getModule().proceedTransaction(transaction, completedTransaction -> {
            });
        }
    }
